    ...
    if filename.endswith(".save"):
        ...
        with open(filename, "rb") as f:
            ...
            for _ in range(quantity):
                ...
                grade = int.from_bytes(f.read(2), byteorder="big")

                add_student(family_name, first_name, grade)
        ...
        with open(filename, "r") as f:
            ...
            for student in data.values():
                add_student(
                    student["family_name"], student["first_name"], student["grade"]
                )
...
def save_file():
    filename = filedialog.asksaveasfilename(
        filetypes=[("Binary", ".save"), ("JSON", ".json")]
    )
    ...
    if filename.endswith(".save"):
        with open(filename, "wb") as f:
            ...
            for student in student_list.values():
                f.write(
                    len(student.family_name.encode("utf-8")).to_bytes(
                        2, byteorder="big"
                    )
                )
                ...
                f.write(
                    len(student.first_name.encode("utf-8")).to_bytes(2, byteorder="big")
                )
        ...
        with open(filename, "w") as f:
            json.dump(
                {key: val.__dict__ for key, val in student_list.items()}, f, indent=2
            )
