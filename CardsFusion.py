...
class Card:
	total = 0
	def __init__(self, name: str, desc: str, cost: int, vars: Dict[str, float], use: Callable[[Any, Any], None], can_use: Callable[[Any, Any], bool]) -> None:
		self.name = name
		self.desc = desc
		self.cost = cost
		self.vars = vars
	...
	def can_use(self, source: Any, target: Any) -> bool:
		return self.can_useFunc(self, source, target) # type: ignore

	def use(self, source: Any, target: Any) -> None:
		return self.useFunc(self, source, target) # type: ignore


class Modifier:
	
	def __init__(self, name: str, priority: Tuple[int, int, int], varsModif: Dict[str, Tuple[str, float]], cost: int = 0, use: Union[str, List[str]] = "", can_use: Union[str, List[str]] = "") -> None:
		self.name = name
