...
def text_dict(entry: Dict[str, int]) -> str:
	...
	return "\n".join([f"{i} x{entry[i]}" for i in sorted(entry.keys())])
...
class GenerationInterface:
	...
	def __str__(self) -> str:
		return f"{self.name}:\n" + "\n".join([f"[{i+1}] {', '.join([f'{j} x{self.content[i][0][j]}' for j in self.content[i][0]])} -> {', '.join([f'{j} x{self.content[i][1][j]}' for j in self.content[i][1]])}" for i in range(len(self.content))])
...
Orchard = CollectionPoint("Orchard", Fruits)
...
while not quitting:
	match input().split():
		case ["collect", x, n]:
			...
			print(f"You collected {n} times in {x} and found:\n{text_dict(output)}\n")
			Bag.add(output)
